...
1. **Mount** — When `active` becomes `true`, the hook queries all focusable elements within the container
2. **Focus cycle** — Tab/Shift+Tab navigation is intercepted to wrap focus between first and last elements
3. **Cleanup** — Event listeners are removed when `active` becomes `false` or on unmount
