...
1. **Configuration** — Accepts an array of action definitions with keys, modifiers, and handlers
2. **Event matching** — On keydown, iterates through actions to find matching key + modifier combination
3. **Action execution** — Invokes the matched action handler, optionally preventing default behavior
4. **Memoization** — Returns a stable callback ref via `useCallback`
...
<TypeTable
  type={{
    ...
    action: {
      ...
      type: 'function',
      parameters: [
        { name: 'e', description: 'React.KeyboardEvent<HTMLElement>' },
      ],
      required: true,
    },
    modifiers: {
